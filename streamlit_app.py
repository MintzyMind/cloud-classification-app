...
if uploaded_image is not None:
    ...
    class_names = ['Altocumulus', 'Altostratus', 'Cirrocumulus', 'Cirrostratus', 'Cirrus', 'Contrails', 'Cumulonimbus', 'Cumulus', 'Nimbostratus', 'Stratocumulus', 'Stratus']
    ...
    if st.button('Prediction'):
        ...
        if 'model' not in locals():
            ...
            for i in range(len(class_names)):
                ...
                st.write(
                    f"## <span style='color:{color}'>{class_names[i]} : {probli[0][i] * 100:.2f}%</span>", unsafe_allow_html=True)
